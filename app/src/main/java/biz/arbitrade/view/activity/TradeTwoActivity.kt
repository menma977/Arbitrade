...
class TradeTwoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        continueBtn.setOnClickListener {
            adapter.add(
                BetHistory(
                    (Random.nextDouble() * Random.nextLong()).toLong(),
                    (Random.nextDouble() * Random.nextLong()).toLong(),
        ...
        initialTask = Timer().scheduleAtFixedRate(betDelay, betPeriod) {
            ...
            if (response.getInt("code") < 400) {
                ...
                betHistory.add(
                    BetHistory(
                        bet,
                        response.getInt("PayOut") - bet,
                        System.currentTimeMillis()
                    )
                )
                bet = controller.martiangle(bet, response.getInt("PayOut") - bet)
                ...
                runOnUiThread {
                    Toast.makeText(
                        applicationContext,
                        "Cannot start trading, please try again later",
                        Toast.LENGTH_LONG
                    ).show()
                    Log.e("TradeTwo.DogeRequest", response.getString("data"))
                    //TODO: make sure!
                    this.cancel()
                }
            }
