...
abstract class Base {
  ...
  fun makePost(url: String, requestBody: FormBody.Builder?, headers: Map<String, String>?): JSONObject {
    return makeRequest(url, "POST", requestBody?.build(), headers)
  }

  fun makeGet(url: String, queries: Map<String, String>?, headers: Map<String, String>?): JSONObject {
    var queryString = ""
    if (!queries.isNullOrEmpty()) queries.forEach { request ->
      queryString += "${request.key}=${request.value}&"
    }
    return makeRequest("$url?$queryString", "GET", null, headers)
  }

  private fun makeRequest(url: String, method: String, body: RequestBody?, headers: Map<String, String>?): JSONObject {
    try {
      ...
      if (!headers.isNullOrEmpty()) headers.forEach { header ->
